...
export async function notifyCustomerProviderAssigned(phone, booking, provider) {
  const reach = booking.mask_number ? ` To reach them, call ${booking.mask_number} (a private WeAid line).` : "";
  const msg = `Great news! ${provider.name} has accepted your ${booking.service} booking #${booking.id} and will arrive at your scheduled time.${reach}`;
  return sendMessage(phone, msg);
}

export async function notifyProviderNewJob(phone, booking) {
  const task = booking.description ? ` — "${booking.description}"` : "";
  const crew = booking.workers > 1 ? ` (${booking.workers}-person job)` : "";
  const earn = booking.provider_earnings ? ` You earn ₹${booking.provider_earnings}.` : "";
  const msg = `New WeAid job! ${booking.service} in ${booking.city}${crew}${task}.${earn} Open your dashboard to accept.`;
  return sendMessage(phone, msg);
}
...
export async function notifyProviderJobConfirmed(phone, booking) {
  ...
  // Masked line if configured — the pro never sees the customer's real number.
  const contact = booking.mask_number
    ? `Call the customer on the private WeAid line: ${booking.mask_number}\n`
    : `Customer: ${booking.customer_name || ""} — ${booking.customer_phone || ""}\n`;
  const msg = `✅ Job confirmed! ${booking.service} in ${booking.city}, ${when}.\n${task}${contact}${addr}You earn ₹${booking.provider_earnings || ""}. Please call to confirm arrival.`;
  return sendMessage(phone, msg);
}
...
export async function notifyDriverNewTrip(phone, trip) {
  const label = trip.type === "courier" ? "Courier pickup" : "Ride request";
  const msg = `🚗 New ${label}! ${trip.pickup_address || "pickup"} → ${trip.drop_address || "drop"} (${trip.distance_km || "?"} km). You earn ₹${trip.provider_earnings || ""}. Open your dashboard to accept.`;
  return sendMessage(phone, msg);
}
