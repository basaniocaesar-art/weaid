...
/* ── Auth: resolve the logged-in user & their provider listing ── */
...
/* ── Provider earnings (marketplace-collected: provider is paid net of commission) ── */
export async function getProviderEarnings(providerId) {
  const { data, error } = await supabase
    .from("bookings")
    .select("provider_earnings, commission, total, status, provider_paid, completed_at")
    .eq("provider_id", providerId)
    .eq("status", "completed");
  if (error) throw error;
  const rows = data || [];
  ...
  for (const b of rows) {
    const net = Number(b.provider_earnings) || 0;
    lifetime += net;
    if (b.completed_at && new Date(b.completed_at) >= startOfMonth) thisMonth += net;
    if (!b.provider_paid) pending += net;
  }
  return { lifetime, thisMonth, pending, jobsDone: rows.length };
}

export async function insertPayoutRequest(row) {
  const { data, error } = await supabase
...
export async function getPayoutRequests(providerId, limit = 20) {
  const { data, error } = await supabase
    .from("payout_requests")
    .select("*")
    .eq("provider_id", providerId)
...
export async function hasOpenPayoutRequest(providerId) {
  const { data, error } = await supabase
    .from("payout_requests")
    .select("id")
    .eq("provider_id", providerId)
